...
restart_img = pygame.transform.scale(restart_img,(60,21))
...
class Enemy(pygame.sprite.Sprite):
	def __init__(self, x, y):
		...
		self.image = pygame.transform.scale(self.image,(23,17))
		self.rect = self.image.get_rect()
		self.rect.x = x
		self.rect.y = y
...
#create buttons
restart_button = Button(screen_width // 2 - 20, screen_height // 2 + 50, restart_img)
	...
	if main_menu == True:
		...
		if game_over == 1:
			...
			if level <= max_levels:
				...
				draw_text('YOU WIN!', font, blue, (screen_width // 2) - 70, screen_height // 2)
				if restart_button.draw():
					level = 1
					#reset level
					world_data = []
